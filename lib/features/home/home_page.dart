...
class HomePage extends StatelessWidget {
    ...
    return PageReveal(child: Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SettingsPage(settings: settings))), icon: const Icon(Icons.tune_rounded), tooltip: 'تنظیمات'),
        ],
      ),
      ...
      body: ListView(
        ...
        children: [
          ...
          ...decks.map((deck) => Padding(padding: const EdgeInsets.only(bottom: 12), child: AnimatedGlassCard(onTap: () async { await Navigator.push(context, MaterialPageRoute(builder: (_) => DeckPage(deck: deck, onChanged: onChanged))); onChanged(); }, child: _deckTile(context, deck)))),
        ],
      ),
    ));
  }
  ...
  Widget _deckTile(BuildContext context, Deck deck) {
    ...
    return Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(deck.name, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800)), const SizedBox(height: 4), Text('${deck.cards.length} کارت • ${dueCount(deck)} کارت آماده مرور', style: TextStyle(color: muted))])),
...
class _HeroBanner extends StatelessWidget {
  final int total, due, goal;
  const _HeroBanner({required this.total, required this.due, required this.goal});
  @override Widget build(BuildContext context) {
    final primary = Theme.of(context).colorScheme.primary;
    final progress = goal <= 0 ? 0.0 : (due / goal).clamp(0.0, 1.0);
    return AnimatedGlassCard(child: Container(decoration: BoxDecoration(gradient: LinearGradient(colors: [primary.withValues(alpha: .14), primary.withValues(alpha: .03)]), borderRadius: BorderRadius.circular(18)), padding: const EdgeInsets.all(16), child: Row(children: [
      Stack(alignment: Alignment.center, children: [SizedBox(width: 64, height: 64, child: CircularProgressIndicator(value: progress, strokeWidth: 7, backgroundColor: primary.withValues(alpha: .10))), Icon(progress >= 1 ? Icons.emoji_events_rounded : Icons.auto_awesome_rounded, color: primary, size: 28)]),
      const SizedBox(width: 16), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(progress >= 1 ? 'هدف امروز کامل شد! 🎉' : 'آماده‌ی یک مرور کوتاه؟', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900)), const SizedBox(height: 5), Text('$due از $goal کارت امروز • مجموع $total کارت', style: TextStyle(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: .60)))])),
    ])));
  }
}
