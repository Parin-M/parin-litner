...
class _StudyPageState extends State<StudyPage>
  ...
  void initState() {
    super.initState();
    queue = List<FlashCard>.from(
      widget.studyCards ??
          widget.deck.cards.where((card) => !card.dueAt.isAfter(DateTime.now())),
    )..shuffle();
  ...
  void _rate(int rating) {
    ...
    switch (rating) {
      ...
      case 2:
        ...
        card.dueAt = now.add(
          Duration(days: [1, 2, 4, 7, 14, 30][min(card.boxIndex, 5)]),
        );
        break;
      default:
        card.boxIndex = min(lastBox, max(0, card.boxIndex + 2));
        card.dueAt = now.add(
          Duration(days: [2, 4, 7, 14, 30, 60][min(card.boxIndex, 5)]),
        );
    }
  ...
  Widget build(BuildContext context) {
    if (queue.isEmpty) {
      return Scaffold(
        ...
        body: const Center(child: Text('در این حالت کارتی برای مرور نداری 🎉')),
      );
    }
          ...
          if (widget.settings.musicEnabled)
            IconButton(
              onPressed: () async {
                if (musicStarted) {
                  await music.pause();
                  if (mounted) setState(() => musicStarted = false);
                } else {
                  await _startMusic();
                }
              },
          ...
          children: [
            ...
            if (revealed) ...[
              ...
              const Padding(
                ...
                child: Text(
                  'کارت را لمس کن تا پاسخ را ببینی.',
                  textDirection: TextDirection.rtl,
                ),
              ),
          ],
...
class _Rate extends StatelessWidget {
  ...
  Widget build(BuildContext context) => FilledButton.tonalIcon(
        ...
        label: Text(text, textDirection: TextDirection.rtl),
      );
}
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              '$count کارت در ${seconds ~/ 60} دقیقه و ${seconds % 60} ثانیه',
              textDirection: TextDirection.rtl,
            ),
            const SizedBox(height: 18),
            FilledButton(
              onPressed: () => Navigator.pop(context),
              child: const Text('عالیه', textDirection: TextDirection.rtl),
            ),
          ],
        ),
      ),
