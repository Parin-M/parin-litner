...
class AppStore extends ChangeNotifier {
  ...
  static Future<AppStore> load() async {
    ...
    final raw = p.getString('decks_v2');
    final decks = raw == null ? _seedDecks() : (jsonDecode(raw) as List).map((e) => Deck.fromJson(Map<String, dynamic>.from(e))).toList();
    return AppStore(
      decks,
      darkMode: p.getBool('dark_v2') ?? true,
      xp: p.getInt('xp_v2') ?? 1250,
      streak: p.getInt('streak_v2') ?? 7,
  ...
  Future<void> save() async {
    final p = await SharedPreferences.getInstance();
    await p.setString('decks_v2', jsonEncode(decks.map((e) => e.toJson()).toList()));
    await p.setBool('dark_v2', darkMode);
    await p.setInt('xp_v2', xp);
    await p.setInt('streak_v2', streak);
...
class LogoMark extends StatelessWidget { const LogoMark({super.key, this.size = 44}); final double size; @override Widget build(BuildContext c) => Container(width: size, height: size, decoration: BoxDecoration(borderRadius: BorderRadius.circular(size * .32), gradient: const LinearGradient(colors: [Color(0xFF73D6FF), Color(0xFF8C69FF), Color(0xFFE47BFF)], begin: Alignment.topLeft, end: Alignment.bottomRight), boxShadow: const [BoxShadow(color: Color(0x668A6CFF), blurRadius: 18)]), child: const Icon(Icons.auto_awesome_rounded, color: Colors.white)); }
