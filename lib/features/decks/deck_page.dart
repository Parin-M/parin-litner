  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.fromLTRB(16, 8, 16, 100), children: [
        ...
        ...cards.map((card) { final boxIndex = deck.boxes.isEmpty ? 0 : min(card.boxIndex, deck.boxes.length - 1); final boxName = deck.boxes.isEmpty ? 'خانه' : deck.boxes[boxIndex].name; return Padding(padding: const EdgeInsets.only(bottom: 10), child: AnimatedGlassCard(onTap: () => _editCard(card), child: Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(card.front.isEmpty ? '🖼️ کارت تصویری' : card.front, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800)), const SizedBox(height: 5), Text(card.back.isEmpty ? '🖼️ پاسخ تصویری' : card.back, maxLines: 2, overflow: TextOverflow.ellipsis), const SizedBox(height: 7), Text('$boxName • ${card.tags}', style: const TextStyle(fontSize: 12))])), IconButton(onPressed: () { card.favorite = !card.favorite; setState(() {}); widget.onChanged(); }, icon: Icon(card.favorite ? Icons.star_rounded : Icons.star_border_rounded, color: card.favorite ? Colors.amber : null))]))); }),
      ]),
    );
  }
...
class _BoxManagerState extends State<BoxManager> {
  ...
  Future<void> _delete(int index) async { if (widget.deck.boxes.length <= 1) return; final removed = widget.deck.boxes.removeAt(index); for (final card in widget.deck.cards) { if (card.boxIndex == index) card.boxIndex = 0; else if (card.boxIndex > index) card.boxIndex--; } setState(() {}); widget.onChanged(); if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${removed.name} حذف شد'))); }
