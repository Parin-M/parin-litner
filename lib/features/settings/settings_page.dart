...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _openRepository() async {
    final uri = Uri.parse('https://github.com/Parin-M/parin-litner');
          ...
          children: [
            ...
            _Section(title: s('appearance'), icon: Icons.palette_outlined, compact: compact, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              SizedBox(height: 92, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: AppTheme.themes.length, separatorBuilder: (_, index) => const SizedBox(width: 9), itemBuilder: (_, index) {
                ...
                final title = _localizedThemeName(widget.settings.languageCode, index, theme.name);
            ...
            _Section(title: s('extra'), icon: Icons.auto_awesome_outlined, compact: compact, child: Column(children: [
              ListTile(contentPadding: EdgeInsets.zero, leading: const CircleAvatar(child: Icon(Icons.workspace_premium_outlined)), title: Text('Achievement Center', style: const TextStyle(fontWeight: FontWeight.w900)), subtitle: const Text('Badges, daily challenges and progress'), trailing: const Icon(Icons.chevron_right), onTap: _openAchievements),
              const Divider(height: 1),
              ListTile(contentPadding: EdgeInsets.zero, leading: const CircleAvatar(child: Icon(Icons.shuffle_rounded)), title: const Text('Smart Study Mix', style: TextStyle(fontWeight: FontWeight.w900)), subtitle: const Text('A future-ready home for smart review modes'), trailing: const Icon(Icons.auto_awesome), onTap: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Smart Study Mix is available in Power Center')))),
              const Divider(height: 1),
  ...
  Widget _switchRow(String key, IconData icon, bool value) => SwitchListTile.adaptive(contentPadding: EdgeInsets.zero, secondary: Icon(icon), title: Text(s(key), style: const TextStyle(fontWeight: FontWeight.w700)), subtitle: Text(s('${key}_sub')), value: value, onChanged: (v) => widget.settings.setBool(key, v));
}
