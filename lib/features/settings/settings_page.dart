...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _languagePicker() async {
    ...
    await showDialog<void>(context: context, builder: (dialogContext) => StatefulBuilder(builder: (dialogContext, setDialogState) {
      ...
      return AlertDialog(title: Text(s('language')), content: SizedBox(width: double.maxFinite, height: 500, child: Column(children: [
        ...
        Expanded(child: ListView.separated(itemCount: list.length, separatorBuilder: (_, index) => const Divider(height: 1), itemBuilder: (_, index) {
          final lang = list[index];
          final active = lang.code == widget.settings.languageCode;
          return ListTile(leading: CircleAvatar(child: Text(lang.code.split('-').first.toUpperCase())), title: Text(lang.nativeName, style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text(lang.name), trailing: active ? Icon(Icons.check_circle, color: Theme.of(dialogContext).colorScheme.primary) : null, onTap: () async { await widget.settings.setLanguage(lang.code); if (dialogContext.mounted) Navigator.pop(dialogContext); });
        })),
  ...
  Future<void> _togglePreview() async {
    try { if (previewPlaying) await music.pause(); else await music.play(widget.settings.musicTrack, volume: widget.settings.musicVolume); if (mounted) setState(() => previewPlaying = !previewPlaying); }
    catch (_) { if (mounted) setState(() => previewPlaying = false); }
  }
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: widget.settings, builder: (context, _) {
    ...
    return Scaffold(appBar: AppBar(title: Text(s('settings'))), body: ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 36), children: [
      ...
      _Section(title: s('appearance'), icon: Icons.palette_outlined, compact: compact, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(s('theme'), style: const TextStyle(fontWeight: FontWeight.w800)), const SizedBox(height: 10),
        SizedBox(height: 92, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: AppTheme.themes.length, separatorBuilder: (_, index) => const SizedBox(width: 9), itemBuilder: (_, index) {
          final theme = AppTheme.themes[index]; final active = index == widget.settings.themeIndex; final title = _localizedThemeName(widget.settings.languageCode, index, theme.name);
      ...
      _Section(title: s('study_preferences'), icon: Icons.school_outlined, compact: compact, child: Column(children: [
        _switchRow('show_xp', Icons.bolt_outlined, widget.settings.showXp), _switchRow('confirm_exit', Icons.exit_to_app_outlined, widget.settings.confirmExit), _switchRow('compact_settings', Icons.density_small_outlined, widget.settings.compactSettings),
      ])),
      const SizedBox(height: 12),
      ...
      _Section(title: s('extra'), icon: Icons.auto_awesome_outlined, compact: compact, child: Column(children: [
        ...
        ListTile(contentPadding: EdgeInsets.zero, leading: const CircleAvatar(child: Icon(Icons.shuffle_rounded)), title: Text(s('smart_mix'), style: const TextStyle(fontWeight: FontWeight.w900)), subtitle: Text(s('smart_mix_sub')), trailing: const Icon(Icons.auto_awesome), onTap: () => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(s('smart_mix_available'))))),
        const Divider(height: 1),
...
String _localizedThemeName(String code, int index, String fallback) {
  const names = <String, List<String>>{
    'en':['Sky','Ocean','Mint','Emerald','Lime','Gold','Coral','Rose','Lilac','Lavender','Indigo','Purple','Red','Earth','Turquoise','Icy Blue','Dark Green','Berry','Graphite','Pure Glass'],
    'es':['Cielo','Océano','Menta','Esmeralda','Lima','Dorado','Coral','Rosa','Lila','Lavanda','Índigo','Púrpura','Rojo','Tierra','Turquesa','Azul hielo','Verde oscuro','Baya','Grafito','Cristal puro'],
    'pt':['Céu','Oceano','Menta','Esmeralda','Lima','Dourado','Coral','Rosa','Lilás','Lavanda','Índigo','Roxo','Vermelho','Terra','Turquesa','Azul gelo','Verde escuro','Frutas vermelhas','Grafite','Vidro puro'],
    'fr':['Ciel','Océan','Menthe','Émeraude','Citron vert','Or','Corail','Rose','Lilas','Lavande','Indigo','Violet','Rouge','Terre','Turquoise','Bleu glacé','Vert foncé','Baie','Graphite','Verre pur'],
    'it':['Cielo','Oceano','Menta','Smeraldo','Lime','Oro','Corallo','Rosa','Lilla','Lavanda','Indaco','Viola','Rosso','Terra','Turchese','Blu ghiaccio','Verde scuro','Frutti di bosco','Grafite','Vetro puro'],
    'tr':['Gökyüzü','Okyanus','Nane','Zümrüt','Limon','Altın','Mercan','Gül','Leylak','Lavanta','İndigo','Mor','Kırmızı','Toprak','Turkuaz','Buz mavisi','Koyu yeşil','Böğürtlen','Grafit','Saf cam'],
    'he':['שמיים','אוקיינוס','מנטה','אמרלד','ליים','זהב','אלמוג','ורד','לילך','לבנדר','אינדיגו','סגול','אדום','אדמה','טורקיז','כחול קרח','ירוק כהה','פירות יער','גרפיט','זכוכית טהורה'],
  };
  final list = names[code] ?? names['en']!;
