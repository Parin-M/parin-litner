...
class _SettingsPageState extends State<SettingsPage> {
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (dialogContext, setDialogState) {
          ...
          final list = AppSettings.languages
              .where((item) => q.isEmpty || '${item.nativeName} ${item.name} ${item.code}'.toLowerCase().contains(q))
              .toList();
  ...
  Future<void> _openRepository() async {
    final opened = await launchUrl(
      Uri.parse('https://github.com/Parin-M/parin-litner'),
      mode: LaunchMode.externalApplication,
    );
    if (!opened && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(s('about_open_error'))),
      );
    }
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SwitchListTile.adaptive(
                      ...
                      onChanged: (v) => widget.settings.setBool('glass', v),
                    ),
                    if (widget.settings.glass)
                      Row(
                        children: [
                          Expanded(
                            child: Slider(
                              value: widget.settings.glassOpacity,
                              min: .30,
                              max: .90,
                              divisions: 12,
                              onChanged: widget.settings.setGlassOpacity,
                            ),
                          ),
                ...
                child: Column(
                  children: [
                    ...
                    ListTile(
                      ...
                      title: Text(s('daily_goal')),
                      trailing: Text('${widget.settings.dailyGoal}'),
                      subtitle: Slider(
                        value: widget.settings.dailyGoal.toDouble(),
                        min: 5,
                        max: 100,
                        divisions: 19,
                        onChanged: (v) => widget.settings.setDailyGoal(v.round()),
                      ),
                    ),
                  ],
                ),
              ...
              _Section(
                ...
                child: Column(
                  children: [
                    SwitchListTile.adaptive(
                      contentPadding: EdgeInsets.zero,
                      title: Text(s('music_study')),
                      value: widget.settings.musicEnabled,
                      onChanged: (v) async {
                        await widget.settings.setBool('music_enabled', v);
                        if (!v) {
                          await music.stop();
                          if (mounted) setState(() => previewPlaying = false);
                        }
                    ...
                    if (widget.settings.musicEnabled) ...[
                      DropdownButtonFormField<String>(
                        ...
                        items: [
                          for (final track in MusicService.tracks)
                            DropdownMenuItem(value: track.id, child: Text(track.title)),
                        ],
                        onChanged: (v) async {
                          if (v != null) {
                            await widget.settings.setMusicTrack(v);
                            if (previewPlaying) {
                              await music.play(v, volume: widget.settings.musicVolume);
                            }
                          }
                        },
                      ),
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Slider(
                              ...
                              divisions: 15,
                              onChanged: (v) async {
                                await widget.settings.setMusicVolume(v);
                                if (previewPlaying) await music.setVolume(v);
                              },
                            ),
                          ),
                      ...
                      Align(
                        ...
                        child: OutlinedButton.icon(
                          ...
                          icon: Icon(previewPlaying ? Icons.pause : Icons.play_arrow),
                ...
                child: Column(
                  children: [
                    ListTile(
                      ...
                      title: Text(s('achievement_center'), style: const TextStyle(fontWeight: FontWeight.w900)),
                      subtitle: Text(s('achievement_center_sub')),
                      trailing: const Icon(Icons.chevron_right),
                    ...
                    ListTile(
                      ...
                      title: Text(s('smart_mix'), style: const TextStyle(fontWeight: FontWeight.w900)),
                      subtitle: Text(s('smart_mix_sub')),
                      trailing: const Icon(Icons.auto_awesome),
                      onTap: () => ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text(s('smart_mix_available'))),
                      ),
                    ),
                    const Divider(height: 1),
                    ListTile(
                      ...
                      title: Text(s('about'), style: const TextStyle(fontWeight: FontWeight.w900)),
                      subtitle: Text(s('about_creator')),
                      trailing: const Icon(Icons.chevron_right),
                    ...
                    ListTile(
                      ...
                      title: Text(s('about_github'), style: const TextStyle(fontWeight: FontWeight.w900)),
                      subtitle: Text(s('about_repo_desc')),
  ...
  Widget _switchRow(String key, IconData icon, bool value) {
    final hasSubtitle = <String>{'animations', 'haptics', 'auto_reveal', 'progress', 'timer'}.contains(key);
    return SwitchListTile.adaptive(
      contentPadding: EdgeInsets.zero,
      secondary: Icon(icon),
      title: Text(s(key), style: const TextStyle(fontWeight: FontWeight.w700)),
      subtitle: hasSubtitle ? null : Text(s('${key}_sub')),
      value: value,
