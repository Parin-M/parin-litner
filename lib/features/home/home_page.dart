            ...
            if (decks.isEmpty)
              const AnimatedGlassCard(
                child: Padding(
                  ...
                  child: Text('هنوز دسته‌ای نساخته‌ای. از دکمه «دسته جدید» شروع کن.', textAlign: TextAlign.center),
                ),
              ),
            ...decks.map(
              (deck) => Padding(
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(colors: [color, Theme.of(context).colorScheme.primary]),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              Text(deck.name, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
              const SizedBox(height: 4),
              Text('${deck.cards.length} کارت • ${dueCount(deck)} کارت آماده مرور', style: TextStyle(color: muted)),
            ],
          ),
        ),
  ...
  Future<void> _newDeck(BuildContext context) async {
    ...
    final name = await showDialog<String>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: TextField(controller: controller, autofocus: true, decoration: const InputDecoration(labelText: 'نام دسته')),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('لغو')),
          FilledButton(onPressed: () => Navigator.pop(context, controller.text.trim()), child: const Text('ساختن')),
        ],
      ),
    );
    controller.dispose();
...
class _HeroBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedGlassCard(
      child: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(colors: [primary.withValues(alpha: .14), primary.withValues(alpha: .03)]),
        ...
        child: Row(
          children: [
            Stack(
              ...
              children: [
                SizedBox(width: 64, height: 64, child: CircularProgressIndicator(value: progress, strokeWidth: 7, backgroundColor: primary.withValues(alpha: .10))),
                Icon(progress >= 1 ? Icons.emoji_events_rounded : Icons.auto_awesome_rounded, color: primary, size: 28),
              ],
            ),
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(progress >= 1 ? 'هدف امروز کامل شد! 🎉' : 'آماده‌ی یک مرور کوتاه؟', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900)),
                  const SizedBox(height: 5),
                  Text('$due از $goal کارت امروز • مجموع $total کارت', style: TextStyle(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: .60))),
                ],
              ),
            ),
          ],
...
class _Stat extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => AnimatedGlassCard(
        child: Row(
          children: [
            ...
            Column(
              ...
              children: [
                ...
                Text(title, style: TextStyle(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: .58))),
              ],
            ),
          ],
        ),
