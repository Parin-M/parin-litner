...
class SafeMemoryImage extends StatelessWidget {
  ...
  final Widget? error;
  const SafeMemoryImage({super.key, required this.base64, this.height, this.width, this.fit = BoxFit.contain, this.borderRadius = BorderRadius.zero, this.error});

  @override
  Widget build(BuildContext context) {
    final bytes = tryDecodeBase64Image(base64);
    if (bytes == null) return error ?? const SizedBox.shrink();
    return ClipRRect(
      ...
      child: Image.memory(
        ...
        errorBuilder: (context, error, stackTrace) => this.error ?? const SizedBox.shrink(),
      ),
    );
  }
}
