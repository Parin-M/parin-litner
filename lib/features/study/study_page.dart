...
class _StudyPageState extends State<StudyPage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (widget.settings.musicEnabled && queue.isNotEmpty) {
      _startMusic();
    }
    if (widget.settings.autoReveal && queue.isNotEmpty) {
      Future.delayed(const Duration(milliseconds: 650), () { if (mounted) _reveal(); });
    }
  }

  Future<void> _startMusic() async {
    try { await music.play(widget.settings.musicTrack, volume: widget.settings.musicVolume); musicStarted = true; } catch (_) {}
  }
  ...
  void _reveal() {
    ...
    if (revealed) flipController.forward(); else flipController.reverse();
  }
  ...
  void _rate(int rating) {
    ...
    switch (rating) {
      case 0: // Again
        ...
        break;
      case 1: // Hard
        card.boxIndex = min(lastBox, max(0, card.boxIndex));
        card.dueAt = now.add(const Duration(days: 1));
        break;
      case 2: // Good
        card.boxIndex = min(lastBox, max(0, card.boxIndex + 1));
        card.dueAt = now.add(Duration(days: [1, 2, 4, 7, 14, 30][min(card.boxIndex, 5)]));
        break;
      default: // Easy
        card.boxIndex = min(lastBox, max(0, card.boxIndex + 2));
  ...
  void _swipe(DragEndDetails details) {
    ...
    if (velocity > 500) _rate(2); else if (velocity < -500) _rate(1);
  }

  Future<void> _showCompletion() async {
    await Future.delayed(const Duration(milliseconds: 260));
    if (!mounted) return;
    await showDialog<void>(context: context, barrierDismissible: false, builder: (_) => _CompletionDialog(count: queue.length, seconds: seconds));
  ...
  @override Widget build(BuildContext context) {
    ...
    final card = queue[index];
    final progress = ((index + (revealed ? .65 : .2)) / queue.length).clamp(0.0, 1.0).toDouble();
    return Scaffold(
      appBar: AppBar(title: Text('${index + 1} / ${queue.length}'), actions: [
      ...
      body: Padding(
        ...
        child: Column(children: [
          ...
          Expanded(child: GestureDetector(onTap: _reveal, onHorizontalDragEnd: _swipe, child: AnimatedBuilder(animation: flipController, builder: (_, __) {
            final angle = flipController.value * pi;
            final isFront = angle < pi / 2;
            final face = isFront
                ? _FlashCardView(key: const ValueKey('front'), text: card.front, image: card.frontImage, revealed: false, label: 'سؤال • برای دیدن پاسخ ضربه بزن')
                : Transform(alignment: Alignment.center, transform: Matrix4.rotationY(pi), child: _FlashCardView(key: const ValueKey('back'), text: card.back, image: card.backImage, revealed: true, label: 'پاسخ • برای برگشت ضربه بزن'));
            return Transform(alignment: Alignment.center, transform: Matrix4.identity()..setEntry(3, 2, 0.0015)..rotateY(angle), child: face);
          }))),
          ...
          if (revealed) ...[
            ...
            Row(children: [Expanded(child: _Rate('دوباره', Icons.refresh_rounded, Colors.redAccent, () => _rate(0))), const SizedBox(width: 7), Expanded(child: _Rate('سخت', Icons.trending_down_rounded, Colors.orange, () => _rate(1))), const SizedBox(width: 7), Expanded(child: _Rate('خوب', Icons.check_rounded, Colors.green, () => _rate(2))), const SizedBox(width: 7), Expanded(child: _Rate('آسان', Icons.bolt_rounded, Colors.cyan, () => _rate(3)))]),
          ] else const Padding(padding: EdgeInsets.only(bottom: 8), child: Text('کارت را لمس کن تا پاسخ را ببینی.', textDirection: TextDirection.rtl)),
        ]),
      ),
    );
  }
...
class _FlashCardView extends StatelessWidget {
  final String text; final String? image; final bool revealed; final String label;
  const _FlashCardView({super.key, required this.text, required this.image, required this.revealed, required this.label});
  @override Widget build(BuildContext context) {
